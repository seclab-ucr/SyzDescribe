        ...
        for (auto &i: b) {
            switch (i.getOpcode()) {
                case llvm::Instruction::Call : {
                    ...
                    if (ci.isDebugOrPseudoInst()) { break; }
                    // e.g., @llvm.lifetime.start.p0i8
                    if (ci.isLifetimeStartOrEnd()) { break; }
                    if (ci.isInlineAsm()) { break; }

                    yhao_dump(-1, i.print, str)
                        ...
                        if (inst->getOpcode() == llvm::Instruction::GetElementPtr) {
                            ...
                            if (gep->isInBounds()) {
                                ...
                                if (se_ty->isStructTy()) {
                                    ...
                                    struct_name = get_real_structure_name(se_ty->getStructName().str());
...
bool sd::T_function::check_call(sd::checker *c, llvm::CallInst *cs) {
    ...
    if (auto *inst = llvm::dyn_cast<llvm::Instruction>(arg)) {
        if (auto *gep = llvm::dyn_cast<llvm::GetElementPtrInst>(inst)) {
            auto se_ty = gep->getSourceElementType();
            if (se_ty->isStructTy()) {
                std::string struct_name = get_real_structure_name(se_ty->getStructName().str());
                ...
                if (auto gep2 = llvm::dyn_cast<llvm::GetElementPtrInst>(arg2)) {
                    ...
                    if (se_ty2->isStructTy()) {
                        std::string struct_name = get_real_structure_name(se_ty2->getStructName().str());
                        if (struct_name == c->structure) {
                            yhao_log(debug, "check_call_cmd: array struct_name: " + struct_name);
                            if (auto c1 = llvm::cast<llvm::ConstantInt>(gep2->getOperand(2))) {
                                uint64_t offset = c1->getZExtValue();
                                if (offset == c->offset) {
                                    return true;
                                }
...
void sd::T_function::check_function_pointer(llvm::Instruction *i) {
    ...
    yhao_log(DEBUG_FUNCTION_POINTER, "check_function_pointer: " + inst_to_strID(i));
...
void
sd::T_function::check_function_pointer(llvm::Value *v, llvm::SmallPtrSet<const llvm::GlobalValue *, 3> &visited) {
    std::string str;
    ...
    while (true) {
        if (auto *gv = llvm::dyn_cast<llvm::GlobalValue>(c)) {
            ...
            if (auto *f = llvm::dyn_cast<llvm::Function>(gv)) {
                yhao_log(DEBUG_FUNCTION_POINTER, "find function pointer: " + f->getName().str());
                ...
                yhao_log(DEBUG_FUNCTION_POINTER, "GlobalVariable: " + gvv->getName().str());
                if (gvv->hasInitializer()) {
                    auto gvvi = gvv->getInitializer();
                    if (const llvm::ConstantVector *cp = llvm::dyn_cast<llvm::ConstantVector>(gvvi)) {
                        for (unsigned ii = 0, e = cp->getNumOperands(); ii != e; ++ii) {
                            check_function_pointer(cp->getOperand(ii), visited);
                        }
                    } else if (const llvm::ConstantArray *ca = llvm::dyn_cast<llvm::ConstantArray>(gvvi)) {
                        for (unsigned ii = 0, e = ca->getNumOperands(); ii != e; ++ii) {
                            check_function_pointer(ca->getOperand(ii), visited);
                        }
                    } else if (const llvm::ConstantStruct *cs = llvm::dyn_cast<llvm::ConstantStruct>(gvvi)) {
                        for (unsigned ii = 0, e = cs->getNumOperands(); ii != e; ++ii) {
                            check_function_pointer(cs->getOperand(ii), visited);
                        }
                    } else if (const llvm::ConstantDataSequential *cds = llvm::dyn_cast<llvm::ConstantDataSequential>(
                            gvvi)) {
            ...
            if (ce->getOpcode() == llvm::Instruction::BitCast) {
                ...
                yhao_log(DEBUG_FUNCTION_POINTER, "check_function_pointer: ConstantExpr: else");
                yhao_dump(DEBUG_FUNCTION_POINTER, ce->print, str)
                return;
            }
        } else if (const llvm::ConstantVector *cp = llvm::dyn_cast<llvm::ConstantVector>(c)) {
            for (unsigned ii = 0, e = cp->getNumOperands(); ii != e; ++ii) {
                check_function_pointer(cp->getOperand(ii), visited);
            }
            return;
        } else if (const llvm::ConstantArray *ca = llvm::dyn_cast<llvm::ConstantArray>(c)) {
            for (unsigned ii = 0, e = ca->getNumOperands(); ii != e; ++ii) {
                check_function_pointer(ca->getOperand(ii), visited);
            }
            return;
            ...
            yhao_dump(DEBUG_FUNCTION_POINTER, c->print, str)
            return;
        }
    }
}

sd::callee *sd::T_function::add_callee(llvm::Instruction *i, sd::callee_type ct, T_function *tf) {
...
sd::callee *sd::T_function::add_callee(llvm::Instruction *i, sd::callee_type ct, std::set<T_function *> &tf) {
    if (tf.empty()) {
        return nullptr;
    }
    auto temp_c = new callee();
...
void sd::T_function::add_checker_result(sd::checker *c, llvm::Instruction *i) {
    ...
    yhao_log(debug, "checker: " + std::to_string(cr1->ct) + " :" + checker_type_name[cr1->ct]);
...
uint64_t sd::T_function::calculate_path(llvm::BasicBlock *start_node, llvm::BasicBlock *end_node,
...
uint64_t sd::T_function::calculate_path(llvm::BasicBlock *start_node, std::set<llvm::BasicBlock *> *end_node,
                                        std::vector<llvm::BasicBlock *> *path) {
    ...
    if (size == 0) {
        ...
        for (auto temp_b1: *end_node) {
            for (auto it = pass_node->begin(); it != pass_node->end(); it++) {
                if (llvm::isPotentiallyReachable(&temp_b1->front(), &(*it)->front(), nullptr, this->dominator_tree)) {
                    pass_node->insert(it, temp_b1);
                    break;
                } else if (llvm::isPotentiallyReachable(&(*it)->front(), &temp_b1->front(), nullptr,
                                                        this->dominator_tree)) {
                    continue;
                } else {
                    ret = 1;
...
uint64_t
sd::T_function::calculate_path(std::set<llvm::Instruction *> *pass_node_i,
                               std::vector<llvm::BasicBlock *> *path) {
    ...
    // filter the basic block by dominate relationship, only keep the dominated basic block
    ...
    for (auto i: *pass_node_i) {
        ...
        if (pass_node_b->find(temp_b) == pass_node_b->end()) {
            for (auto b: *pass_node_b) {
                // only insert dominated basic block, so all those basic blocks would be reached
    ...
    uint64_t ret = calculate_path(&(this->llvm_function->getEntryBlock()), pass_node_b, path);
    return ret;
}

uint64_t sd::T_function::calculate_path(std::set<llvm::Instruction *> *pass_node_i, std::vector<uint64_t> *path) {
...
void sd::T_function::add_callee_checker_results(llvm::Instruction *i, sd::T_function *t_f) {
