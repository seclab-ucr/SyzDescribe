...
namespace sd {
    ...
    public:
        ioctl_cmd_type(llvm::Function *tf,
        ...
        void get_arg_propagation(llvm::CallInst *ci, std::set<uint64_t> &cmd, std::set<uint64_t> &type,
                                 std::map<uint64_t, llvm::Value *> &caller_arg);

        static llvm::Type *typeOutputHandler(llvm::Value *val, ioctl_cmd_type *currFunc);

        static llvm::Type *get_type_recursive(llvm::Value *val, std::set<llvm::Instruction *> &visited);

        static llvm::Value *get_inst_recursive(llvm::Value *val, std::set<llvm::Instruction *> &visited);
