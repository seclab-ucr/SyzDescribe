...
void sd::Manager::setup(const std::string &config) {
    ...
    if (config_json.contains("4_work_dir") && config_json["4_work_dir"].is_string()) {
...
void sd::Manager::analysis() {
    ...
    for (auto &st: t_module->llvm_module->getIdentifiedStructTypes()) {
        ...
        if (struct_name == "struct.device") {
            ...
            for (auto et: st->elements()) {
                if (et->isStructTy()) {
                    std::string element_name = get_real_structure_name(et->getStructName().str());
    ...
    yhao_log(1, "print knowledge json file: " + this->work_dir + "knowledge.json");
    ...
    for (const auto &dd: this->device_drivers) {
        if (dd.second->syscalls.find(syscall_name[3]) != dd.second->syscalls.end()) {
            this->number_ioctl++;
        }
    }
    ...
    for (const auto &init_f: this->init_function) {
        ...
        this->number_indirect_call_target_before += init_f.second->number_indirect_call_target_before;
        this->number_indirect_call_target_after += init_f.second->number_indirect_call_target_after;
    ...
    for (const auto &init_f: this->all_new_entry_functions) {
        ...
        this->number_indirect_call_target_before += init_f->number_indirect_call_target_before;
        this->number_indirect_call_target_after += init_f->number_indirect_call_target_after;
    ...
    for (const auto &dd_pair: this->device_drivers) {
        ...
        statistic << " " + std::to_string(dd->syscalls.find(syscall_name[3]) != dd->syscalls.end());
...
void sd::Manager::collect_info(device_driver *dd) {
    ...
    if (dd->non_open_parent != nullptr) {
        yhao_log(debug, "non_open_parent: " + std::to_string(dd->non_open_parent->value));
    }
...
void sd::Manager::collect_entry_function(device_driver *dd, std::set<entry_function *> &entry_set) {
    if (dd->syscalls.find(syscall_name[3]) != dd->syscalls.end() && dd->syscalls[syscall_name[3]] != nullptr) {
...
void sd::Manager::analysis_init_function() {
    ...
    for (const auto &temp: this->t_module->module_init_function) {
        ...
        for (auto f: *set_f->f) {
            ...
            if (this->t_module->t_functions.find(f) == this->t_module->t_functions.end()) {
                yhao_log(3, "analysis_init_functions can not find function: " + f->getName().str());
            } else {
                ...
                if (results.empty()) {
                    ...
                    for (auto temp1: results) {
                        ...
                        if (kind & (1 << checker_type::dev_name)) {
                            yhao_log(debug, "init_function_with_driver_device dev_name: " + f->getName().str());
                            init_function_with_driver_device[f->getName().str()] = init_f;
                        } else if (kind & (1 << checker_type::dri_name)) {
                            yhao_log(debug, "init_function_with_driver_device dri_name: " + f->getName().str());
                            init_function_with_driver_device[f->getName().str()] = init_f;
                        } else {
                            yhao_log(debug, "init_function_with_driver: " + f->getName().str());
    ...
    yhao_log(1, "init_function_with_driver: " + std::to_string(init_function_with_driver.size()));
    yhao_log(1, "init_function_with_device: " + std::to_string(init_function_with_device.size()));
    yhao_log(1, "init_function_with_driver_device: " + std::to_string(init_function_with_driver_device.size()));
...
int64_t sd::Manager::handle_init_function_with_driver() {
    ...
    for (auto temp: this->all_device_drivers) {
        ...
        yhao_dump(-1, cr_open->inst->print, str)
        auto oc = new offset_chain_base();
        ...
        if (results.empty()) {
            ...
            for (auto temp1: results) {
                ...
                if (kind & (1 << checker_type::dev_name)) {
                    ...
                    yhao_log(1, "new init_function_with_driver_device dev_name: " + init_f->get_entry_function_name());
                    init_function_with_driver_device[init_f->get_entry_function_name()] = init_f;
                } else if (kind & (1 << checker_type::dri_name)) {
                    temp[f.first] = f.second;
                    yhao_log(1, "new init_function_with_driver_device dri_name: " + init_f->get_entry_function_name());
                    init_function_with_driver_device[init_f->get_entry_function_name()] = init_f;
                } else {
...
int64_t sd::Manager::handle_init_function_with_driver_device() {
    yhao_log(1, "****handle_init_function_with_driver_device start*******************");
    int64_t debug = -1;
    std::string str;
    int64_t ret = 0;
    ...
    yhao_log(1, "****handle_init_function_with_driver_device end*******************");
    return ret;
}

void sd::Manager::mapping_checker_results(entry_function *ef, std::vector<device_driver *> &results) {
    int64_t debug = -1;
    ...
    for (auto c: crs) {
        std::string p1 = get_file_name(c->call_chain.at(0)->getParent()->getParent());
        ...
        for (auto temp: all_device_drivers) {
            if (c->id == temp->ops->id) {
                ...
                if (temp_match > 0) {
                    ...
                    std::string p2 = get_file_name(temp->ops->call_chain.at(0)->getParent()->getParent());
...
void sd::Manager::check_cmd_and_type(const std::string &function_name, nlohmann::json *result_json,
                                     std::set<llvm::Function *> *fps) const {


}

int64_t sd::Manager::get_function_by_index(sd::checker_result_ops *cr0, int64_t index, llvm::Function **fp) const {
    std::string str;
    int64_t debug = -1;
    int64_t ret;
    llvm::GlobalVariable *ops;
    ret = this->t_module->find_ops_structure(cr0->ops_structure, cr0->ops_name, &ops);
    if (ret) {
        return 1;
    }
    ...
    if (cs == nullptr) {
        yhao_log(debug, "llvm::dyn_cast<llvm::ConstantStruct>");
    ...
    if (index >= cs->getNumOperands() || index < 0) {
        ...
        yhao_log(2, "get_function_by_index: index >= cs->getNumOperands(): " + std::to_string(index));
        return 1;
    }
    ...
    if (fp2 == nullptr) {
        ...
        yhao_log(debug, "get_function_by_index: fail: " + std::to_string(index) + ": " + str);
        return 1;
    }
    *fp = fp2;
    return 0;
...
int64_t sd::Manager::get_function_by_syscall(sd::checker_result_ops *cr0, std::string syscall, int64_t *index,
...
sd::task *
sd::Manager::make_task(const std::string &entry_function, checker_result *cr,
...
void sd::Manager::execute_init_function(std::vector<device_driver *> &dds) const {
    for (auto temp_dd: dds) {
        ...
        for (auto temp_1: temp_dd->name) {
            auto task2 = make_task(temp_dd->parent->get_entry_function_name(), temp_1);
            execute_task(task2);
        }
    }
}
...
//            cr = new checker_result_number();
        ...
        case checker_type::dri_name: {
            ...
            if (inst->getOpcode() == llvm::Instruction::Call) {
                ...
                if (cr->value_offset != -2) {
                    ...
                    v = temp_v1->getOperand(cr->value_offset);
                }
                if (auto temp_v1 = llvm::dyn_cast<llvm::GEPOperator>(v);temp_v1 == nullptr) {
                    break;
                } else {
                ...
                auto gvI = gv->getInitializer();
                const llvm::ConstantDataArray *currDArray = llvm::dyn_cast<llvm::ConstantDataArray>(gvI);
                if (currDArray != nullptr) {
                    cr->fmt = currDArray->getAsCString().str();
                } else {
                    yhao_print(-1, gvI->print, str)
                    cr->fmt = str;
                }
                ...
                yhao_log(debug, "number of call inst: " + std::to_string(inst->getNumOperands()));
                for (int64_t i = cr->value + 1; i < inst->getNumOperands() - 1; ++i) {
                    ...
                    if (llvm::isa<llvm::GEPOperator>(temp_v1)) {
                        ...
                        const llvm::ConstantDataArray *temp_currDArray = llvm::dyn_cast<llvm::ConstantDataArray>(
                                temp_gv1I);
                        if (temp_currDArray != nullptr) {
                            cr->va2.push_back(temp_currDArray->getAsCString().str());
                        } else {
                            yhao_print(-1, temp_gv1I->print, str)
        ...
        case checker_type::file_ops: {
            ...
            if (inst->getOpcode() == llvm::Instruction::Store) {
                ...
                auto gv = llvm::dyn_cast<llvm::GlobalVariable>(v);
                if (gv == nullptr) {
                    yhao_log(2, "gv == nullptr");
                    break;
                }
                if (!gv->hasInitializer()) {
                    yhao_log(2, "!gv->hasInitializer()");
                    break;
                }
                auto gvI = gv->getInitializer();
                if (cr->value_offset != -2) {
                    ...
                    auto temp_v2 = temp_v1->getOperand(cr->value_offset);
                    gv = llvm::dyn_cast<llvm::GlobalVariable>(temp_v2);
                    if (gv == nullptr) {
                        yhao_log(1, "gv == nullptr");
                        break;
                    }
                    if (!gv->hasInitializer()) {
                        yhao_log(1, "!gv->hasInitializer()");
                        break;
                    }
...
void sd::Manager::collect_entry(sd::device_driver *dd) const {
    int64_t ret;
    llvm::GlobalVariable *ops;
    ret = this->t_module->find_ops_structure(dd->ops->ops_structure, dd->ops->ops_name, &ops);
    if (ret) {
        return;
    }
