...
namespace sd {
    ...
    public:
        ...
        static void collect_entry_function(device_driver *dd, std::set<entry_function *> &entry_set);
        ...
        void mapping_checker_results(entry_function *ef, std::vector<device_driver *> &results);

        void check_cmd_and_type(const std::string &function_name, nlohmann::json *result_json,
        ...
        int64_t get_function_by_index(sd::checker_result_ops *cr0, int64_t index, llvm::Function **fp) const;

        int64_t get_function_by_syscall(sd::checker_result_ops *cr0, std::string syscall, int64_t *index,
                                        llvm::Function **fp) const;

        // make a task_str based on give information
        // use default information from Class Manager
        [[nodiscard]] task *
        make_task(const std::string &entry_function, checker_result *cr,
