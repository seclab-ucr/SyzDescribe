...
std::string get_real_function_type(llvm::FunctionType *ft) {
  ...
  if (ret->isPointerTy() && ret->getNumContainedTypes() &&
      ret->getNonOpaquePointerElementType()->isStructTy()) {
    yhao_print(debug, ret->print, str) str = str.substr(0, str.size() - 1);
    ...
    yhao_print(debug, ret->print, str) type += str;
  }
  type += " (";
  for (auto i = 0; i < ft->getNumParams(); i++) {
    ...
    if (temp->isPointerTy() && temp->getNumContainedTypes() &&
        temp->getNonOpaquePointerElementType()->isStructTy()) {
      yhao_print(debug, temp->print, str) str = str.substr(0, str.size() - 1);
      if (str.find("struct") == std::string::npos) {
        type += str;
      } else {
        type += (get_real_structure_name(str) + "*");
      ...
      yhao_print(debug, temp->print, str) type += str;
    }
  }
  type += ")";
  return type;
