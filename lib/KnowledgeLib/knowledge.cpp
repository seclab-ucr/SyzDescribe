...
int64_t sd::K_base::find_k_variable(const std::string &structure1, int64_t offset, sd::K_variable **k) {
...
int64_t sd::K_driver::find_k_variable(const std::string &structure1, int64_t offset, sd::K_variable **k) {
    int64_t ret;
...
void sd::knowledge::read(const std::string &knowledge) {
    ...
    if (knowledge_json.contains("device") && knowledge_json["device"].is_object()) {
        this->device = new K_device(knowledge_json["device"]);
    } else {
        yhao_log(3, "knowledge_json.contains(device)");
    }
    if (knowledge_json.contains("driver") && knowledge_json["driver"].is_array()) {
...
void sd::knowledge::make_checker(checker_type type, K_base *kb, K_variable *kv, uint64_t id) {
    if (kv == nullptr)
        return;
...
void sd::knowledge::make_driver_checker(const std::vector<K_driver *> &drivers) {
    for (auto d: drivers) {
        uint64_t id = 0;
...
int64_t sd::knowledge::add_knowledge(sd::checker_result *cr, std::string key_dereference_obj_name,
...
int64_t sd::knowledge::find_k_variable(const std::string &structure, int64_t offset, sd::K_variable **k) {
    uint64_t ret;
    ...
    yhao_log(3, "not find K_variable: " + structure + ": " + std::to_string(offset));
    return 1;
}
