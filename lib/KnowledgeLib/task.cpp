...
sd::task::task(nlohmann::json j) {
    this->from_json(std::move(j));
}

sd::task::task(const std::string &entryFunction,
               sd::checker_result *cr) {
...
void sd::task::update_checker_inst(llvm::Module *m) {
    this->cr->setInst(m);
}
